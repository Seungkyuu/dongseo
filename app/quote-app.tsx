...
const COMPANY_LEGAL = "이준성 대표 · 서울특별시 강서구 마곡중앙6로 10, 203-23호";
/** 사업자등록번호는 하이픈에서 줄이 갈리지 않게 따로 떼어 nowrap으로 렌더한다. */
const COMPANY_REG = "사업자등록번호 309-86-04116";
...
/**
 * 로고 마크 — 두 갈래 길이 만나 하나로 합류하는 모양. "서로 + 동행"
 * (서동) 컨셉을 도상화 — 각자 다른 자리에서 출발한 두 선이 원 안에서
 * 하나로 이어져 함께 내려간다.
 */
function LogoMark({ size = 26 }: { size?: number }) {
  return (
    ...
    >
      ...
      <path
        d="M12.5,12.5 C12.5,19 15.7,22.1 20,24"
        stroke="currentColor"
        strokeWidth="2.6"
        strokeLinecap="round"
        fill="none"
      />
      <path
        d="M27.5,12.5 C27.5,19 24.3,22.1 20,24"
        className="logo-mark-accent"
        strokeWidth="2.6"
        strokeLinecap="round"
        fill="none"
      />
      <path d="M20,24 L20,31" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" />
